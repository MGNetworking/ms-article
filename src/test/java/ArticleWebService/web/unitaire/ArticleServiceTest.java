...
public class ArticleServiceTest {
    ...
    public void testServiceMockThrowsException() {
        ...
        ArticleException exception = Assertions.assertThrows(ArticleException.class, () -> {
            articleService.findArticleById(9999);
        });
        ...
        Mockito.verify(articleRepository, Mockito.times(1)).findById(invalidId);

    }

    @Test
    @DisplayName("Lève une exception ArticleException pour une erreur technique (DataAccessException)")
    void testFindArticleById_DataAccessException() {
        // Arrange
        Integer id = 1;
    ...
    @Test
    @DisplayName("Retourne un article valide pour un ID existant")
    void testFindArticleById_ReturnsArticle() throws ArticleException {
        // Arrange
        ...
        Mockito.when(articleRepository.findById(validId)).thenReturn(Optional.of(mockArticle));

        // Act
        Article result = articleService.findArticleById(validId);

        // Assert
        Assertions.assertNotNull(result, "L'article ne doit pas être null");
        Assertions.assertEquals(validId, result.getIdArticle(), "L'ID de l'article doit correspondre");
        Assertions.assertEquals("Article Test", result.getTitre(), "Le titre de l'article doit correspondre");
        Assertions.assertEquals("Contenu de l'article de test.", result.getArticle(), "Le contenu de l'article doit correspondre");

        // Vérifie que le repository a bien été appelé
        Mockito.verify(articleRepository, Mockito.times(1)).findById(validId);
    }
